...
#     max_prob_confidence = np.max(probabilities)
    
#     # Gap between top two probabilities
#     sorted_probs = np.sort(probabilities)[::-1]
#     gap_confidence = sorted_probs[0] - sorted_probs[1]
    
#     return entropy_confidence, max_prob_confidence, gap_confidence
...
def calculate_confidence_metrics(probabilities):
    ...
    max_prob_confidence = np.max(probabilities)
    sorted_probs = np.sort(probabilities)[::-1]
    gap_confidence = sorted_probs[0] - sorted_probs[1] if len(sorted_probs) > 1 else 0
...
#         ensemble_df[col] = 0.0  # Initialize with zeros
...
#             entropy_conf, max_prob_conf, gap_conf = calculate_confidence_metrics(probs)
            
#             # Combine confidence metrics (you can adjust these weights)
#             confidence = 0.4 * entropy_conf + 0.4 * max_prob_conf + 0.2 * gap_conf
...
#         # Update ensemble_df with the final probabilities
#         for j, col in enumerate(class_columns):
#             ensemble_df.at[i, col] = final_probs[j]
        
#         # Set the prediction to the class with highest probability
...
def ensemble_predictions(dfs, class_columns):
    ...
    for i in range(len(ensemble_df)):
        ...
        for df_index, df in enumerate(dfs):
            ...
            entropy_conf, max_prob_conf, gap_conf = calculate_confidence_metrics(probs)
            confidence = 0.5 * entropy_conf + 0.2 * max_prob_conf + 0.3 * gap_conf
